...
class CompareMode : public QObject
{
  ...
  public:
    ...
    // 指定codeid去打开文件;
    static CodeId scanFileOutputByCodeId(CodeId cid, QString filepath, std::vector<FileLineInfo>& lineInfoVct, int& nMaxLineSize, int& charNums);

    static CodeId readLineFromFile(uchar* fileptr, const int fileLength, const CodeId cid, std::vector<FileLineInfo>& lineInfoVct, int& nMaxLineSize, int& charNums);
